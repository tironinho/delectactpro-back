...
import { uuid, nowIso, generateApiSecret, encryptSecret, decryptSecret, maskSecret } from '../util.js'
import { requireAuth, requireRole } from '../auth.js'
import { createCustomerApiSchema, patchCustomerApiSchema } from '../schemas/customerApis.js'
import { env } from '../env.js'
import { testHealth, testStatus, testDelete, type CustomerApiConfig } from '../services/customerApiClient.js'
...
function buildConfig(row: Row, sharedSecretPlain?: string | null, bearerTokenPlain?: string | null): CustomerApiConfig {
  return {
    baseUrl: row.base_url,
    healthPath: row.health_path,
    deletePath: row.delete_path,
    statusPath: row.status_path,
    authType: row.auth_type as CustomerApiConfig['authType'],
...
export function createCustomerApisRouter(db: DB): Router {
  const router = Router()
  ...
  router.post('/', requireAuth, requireRole('OWNER', 'ADMIN'), (req, res) => {
    ...
    const id = uuid()
    const now = nowIso()
    let sharedSecretPlain: string | null = null
    let sharedSecretEncrypted: string | null = null
    ...
    `).run(
      id, orgId, d.name, d.baseUrl,
      d.healthPath ?? '/deleteactpro/health',
      d.deletePath ?? '/deleteactpro/delete',
      d.statusPath ?? '/deleteactpro/status',
      d.webhookPath ?? null,
      d.authType,
      sharedSecretEncrypted,
      bearerTokenEncrypted,
      d.headers ? JSON.stringify(d.headers) : null,
      d.timeoutMs ?? 8000,
      d.retries ?? 2,
      d.hmacHeaderName ?? 'X-DAP-Signature',
      d.timestampHeaderName ?? 'X-DAP-Timestamp',
      d.replayWindowSeconds ?? 300,
      now, now
    )

    const row = db.prepare('SELECT * FROM customer_api_integrations WHERE id = ?').get(id) as Row
  ...
  router.patch('/:id', requireAuth, requireRole('OWNER', 'ADMIN'), (req, res) => {
    ...
    const existing = db.prepare('SELECT * FROM customer_api_integrations WHERE id = ? AND org_id = ?').get(id, orgId) as Row | undefined
    ...
    if (d.name !== undefined) { updates.push('name = ?'); params.push(d.name) }
    if (d.baseUrl !== undefined) { updates.push('base_url = ?'); params.push(d.baseUrl) }
    if (d.healthPath !== undefined) { updates.push('health_path = ?'); params.push(d.healthPath) }
    if (d.deletePath !== undefined) { updates.push('delete_path = ?'); params.push(d.deletePath) }
    if (d.statusPath !== undefined) { updates.push('status_path = ?'); params.push(d.statusPath) }
    if (d.webhookPath !== undefined) { updates.push('webhook_path = ?'); params.push(d.webhookPath) }
    if (d.authType !== undefined) { updates.push('auth_type = ?'); params.push(d.authType) }
    if (d.headers !== undefined) { updates.push('headers_json = ?'); params.push(d.headers ? JSON.stringify(d.headers) : null) }
    if (d.timeoutMs !== undefined) { updates.push('timeout_ms = ?'); params.push(d.timeoutMs) }
    if (d.retries !== undefined) { updates.push('retries = ?'); params.push(d.retries) }
    if (d.hmacHeaderName !== undefined) { updates.push('hmac_header_name = ?'); params.push(d.hmacHeaderName) }
    if (d.timestampHeaderName !== undefined) { updates.push('timestamp_header_name = ?'); params.push(d.timestampHeaderName) }
    if (d.replayWindowSeconds !== undefined) { updates.push('replay_window_seconds = ?'); params.push(d.replayWindowSeconds) }
  ...
  function getRow(orgId: string, id: string): Row | undefined {
    return db.prepare('SELECT * FROM customer_api_integrations WHERE id = ? AND org_id = ?').get(id, orgId) as Row | undefined
  }
  ...
  router.post('/:id/test-health', requireAuth, requireRole('OWNER', 'ADMIN'), async (req, res) => {
    ...
    let sharedPlain: string | null = null
    let bearerPlain: string | null = null
  ...
  router.post('/:id/test-status', requireAuth, requireRole('OWNER', 'ADMIN'), async (req, res) => {
    ...
    let bearerPlain: string | null = null
    if (row.auth_type === 'HMAC' && row.shared_secret_encrypted) {
      try { sharedPlain = decryptSecret(row.shared_secret_encrypted) } catch {
        return res.status(500).json({ error: 'Decryption failed (check APP_ENCRYPTION_KEY)' })
      }
    }
    if (row.auth_type === 'BEARER' && row.bearer_token_encrypted) {
      try { bearerPlain = decryptSecret(row.bearer_token_encrypted) } catch {
        return res.status(500).json({ error: 'Decryption failed (check APP_ENCRYPTION_KEY)' })
      }
    }
    const config = buildConfig(row, sharedPlain, bearerPlain)
  ...
  router.post('/:id/test-delete', requireAuth, requireRole('OWNER', 'ADMIN'), async (req, res) => {
    const orgId = req.user!.org_id
    const id = req.params.id
    const body = (req.body || {}) as { requestId?: string; subjectHash?: string }
    const row = getRow(orgId, id)
    if (!row) return res.status(404).json({ error: 'Not found' })
    let sharedPlain: string | null = null
    let bearerPlain: string | null = null
