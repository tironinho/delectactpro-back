...
export function createOnboardingRouter(db: DB): Router {
  ...
  router.get('/status', requireAuth, (req, res) => {
    ...
    const org = db.prepare('SELECT id, name, setup_fee_paid_at FROM orgs WHERE id = ?').get(orgId) as { id: string; name: string; setup_fee_paid_at: string | null } | undefined
    const connectors = db.prepare('SELECT id, last_heartbeat_at FROM connectors WHERE org_id = ?').all(orgId) as Array<{ id: string; last_heartbeat_at: string | null }>
    const customerApis = db.prepare('SELECT id, last_healthcheck_at, last_healthcheck_ok FROM customer_api_integrations WHERE org_id = ?').all(orgId) as Array<{ id: string; last_healthcheck_at: string | null; last_healthcheck_ok: number | null }>
    const hashRecipe = db.prepare('SELECT id FROM hash_recipes WHERE org_id = ? AND active = 1').get(orgId) as { id: string } | undefined
    const partners = db.prepare('SELECT id FROM partners WHERE org_id = ?').all(orgId) as Array<{ id: string }>
    const targets = db.prepare('SELECT id FROM partner_targets WHERE org_id = ?').all(orgId) as Array<{ id: string }>
    const legacyPolicies = db.prepare('SELECT id FROM cascade_policies WHERE org_id = ?').all(orgId) as Array<{ id: string }>
    const v2Policies = db.prepare('SELECT id FROM cascade_policies_v2 WHERE org_id = ?').all(orgId) as Array<{ id: string }>
    const lastRun = db.prepare(`
      SELECT id, ended_at, status FROM runs WHERE org_id = ? AND status = 'COMPLETED' ORDER BY ended_at DESC LIMIT 1
    `).get(orgId) as { id: string; ended_at: string | null; status: string } | undefined
    ...
    const completed = stepKeys.filter((k) => (steps[k] as { complete?: boolean }).complete).length
    const readinessScore = stepKeys.length > 0 ? Math.round((completed / stepKeys.length) * 100) : 0

    const blockers: string[] = []
    if (!orgComplete) blockers.push('Organization not set')
    if (!integrationsComplete) blockers.push('At least one integration (connector or customer API) required')
    if (!hashRecipeComplete) blockers.push('No active hash recipe')
    if (!partnersComplete) blockers.push('At least one partner required')
    if (!policiesComplete) blockers.push('At least one cascade policy required')
    if (!dryRunComplete) blockers.push('Complete a dry run within the last 45 days')
    if (!billingComplete) blockers.push('Setup fee not paid')
  ...
  router.post('/validate', requireAuth, (req, res) => {
    ...
    `).get(orgId) as { ended_at: string | null } | undefined
    const setupFee = db.prepare('SELECT setup_fee_paid_at FROM orgs WHERE id = ?').get(orgId) as { setup_fee_paid_at: string | null } | undefined
