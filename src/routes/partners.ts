...
export function createPartnersRouter(db: DB): Router {
  const router = Router()
  ...
  router.get('/', requireAuth, (req, res) => {
    ...
    const rows = db.prepare(
      'SELECT id, org_id, name, type, endpoint_url, enabled, created_at FROM partners WHERE org_id = ? ORDER BY created_at DESC'
    ).all(orgId) as Array<{ id: string; org_id: string; name: string; type: string | null; endpoint_url: string; enabled: number; created_at: string }>
    return res.json({
      items: rows.map((r) => ({
        id: r.id,
        orgId: r.org_id,
  ...
  router.patch('/:id', requireAuth, requireRole('OWNER', 'ADMIN'), (req, res) => {
    ...
    const now = nowIso()
    const d = parsed.data
    if (d.name !== undefined) db.prepare('UPDATE partners SET name = ?, type = COALESCE(?, type), endpoint_url = COALESCE(?, endpoint_url), enabled = COALESCE(?, enabled) WHERE id = ? AND org_id = ?').run(d.name, d.type ?? null, d.endpointUrl ?? null, d.enabled === undefined ? undefined : d.enabled ? 1 : 0, id, orgId)
    else if (Object.keys(d).length) {
      const updates: string[] = []
      const params: unknown[] = []
      if (d.type !== undefined) { updates.push('type = ?'); params.push(d.type) }
      if (d.endpointUrl !== undefined) { updates.push('endpoint_url = ?'); params.push(d.endpointUrl) }
      if (d.enabled !== undefined) { updates.push('enabled = ?'); params.push(d.enabled ? 1 : 0) }
      if (params.length) {
        params.push(id, orgId)
        db.prepare(`UPDATE partners SET ${updates.join(', ')} WHERE id = ? AND org_id = ?`).run(...params)
      }
    }
    const row = db.prepare('SELECT id, org_id, name, type, endpoint_url, enabled, created_at FROM partners WHERE id = ?').get(id) as { id: string; org_id: string; name: string; type: string | null; endpoint_url: string; enabled: number; created_at: string }
  ...
  router.get('/links', requireAuth, (req, res) => {
    ...
    `).all(orgId) as Array<{ id: string; org_id: string; partner_id: string; connector_id: string }>
  ...
  router.post('/links', requireAuth, requireRole('OWNER', 'ADMIN'), (req, res) => {
    ...
    try {
      ...
      if (String((e as { message?: string })?.message).includes('FOREIGN KEY')) return res.status(400).json({ error: 'Partner or connector not found' })
      if (String((e as { message?: string })?.message).includes('UNIQUE')) return res.status(409).json({ error: 'Link already exists' })
      throw e
    }
  ...
  router.post('/policies', requireAuth, requireRole('OWNER', 'ADMIN'), (req, res) => {
    ...
    try {
      db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(
        id, orgId, parsed.data.partnerId, parsed.data.connectorId, parsed.data.mode,
        parsed.data.retriesMax ?? 3, parsed.data.backoffMinutes ?? 60, parsed.data.slaDays ?? null,
        parsed.data.attestationRequired ? 1 : 0, parsed.data.escalationEmail ?? null, now
      )
    } catch (e: unknown) {
      if (String((e as { message?: string })?.message).includes('FOREIGN KEY')) return res.status(400).json({ error: 'Partner or connector not found' })
      throw e
  ...
  router.patch('/policies/:id', requireAuth, requireRole('OWNER', 'ADMIN'), (req, res) => {
    ...
    if (d.connectorId !== undefined) { updates.push('connector_id = ?'); params.push(d.connectorId) }
    ...
    const row = db.prepare('SELECT * FROM cascade_policies WHERE id = ?').get(id) as { id: string; org_id: string; partner_id: string; connector_id: string; mode: string; retries_max: number; backoff_minutes: number; sla_days: number | null; attestation_required: number; escalation_email: string | null; created_at: string }
    return res.json({
