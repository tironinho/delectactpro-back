export function createCustomerApiIntegrationsRepositorySqlite(db) {
    return {
      ...
      `).run(row.id, row.org_id, row.name, row.base_url, row.health_path, row.delete_path, row.status_path, row.webhook_path, row.auth_type, row.shared_secret_encrypted, row.bearer_token_encrypted, row.headers_json, row.timeout_ms, row.retries, row.hmac_header_name, row.timestamp_header_name, row.replay_window_seconds, row.created_at, row.updated_at);
        },
