...
export function createConnectorsRouter(db) {
    const router = Router();
    ...
    router.post('/', requireAuth, requireRole('OWNER', 'ADMIN'), (req, res) => {
        ...
        const tokenId = uuid();
        db.transaction(() => {
            db.prepare('INSERT INTO connectors (id, org_id, name, db_type, created_at, last_heartbeat_at, agent_version, status) VALUES (?, ?, ?, ?, ?, NULL, NULL, ?)').run(id, orgId, parsed.data.name, parsed.data.dbType, createdAt, 'PENDING');
            db.prepare('INSERT INTO connector_tokens (id, connector_id, token_hash, created_at, revoked_at) VALUES (?, ?, ?, ?, NULL)').run(tokenId, id, tokenHash, createdAt);
        })();
    ...
    router.post('/:id/rotate-token', requireAuth, requireRole('OWNER', 'ADMIN'), (req, res) => {
        ...
        const tokenId = uuid();
        const now = nowIso();
        db.prepare('INSERT INTO connector_tokens (id, connector_id, token_hash, created_at, revoked_at) VALUES (?, ?, ?, ?, NULL)').run(tokenId, connectorId, tokenHash, now);
