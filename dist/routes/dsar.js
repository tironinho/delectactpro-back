...
export function createDsarRouter(db) {
    const router = Router();
    ...
    router.post('/requests', requireAuth, (req, res) => {
      ...
      VALUES (?, ?, ?, ?, ?, ?, 'RECEIVED', ?, ?, ?)
    `).run(id, orgId, body.requestRef ?? null, body.subjectHash.toLowerCase(), body.payloadHash?.toLowerCase() ?? null, body.system?.slice(0, 80) ?? 'drop', receivedAt, body.meta ? JSON.stringify(body.meta).slice(0, 50_000) : null, createdAt);
        db.prepare(`
    ...
    router.post('/requests/:id/events', requireAuth, (req, res) => {
        const orgId = req.user.org_id;
        const id = req.params.id;
        const { type, actor, details } = (req.body || {});
