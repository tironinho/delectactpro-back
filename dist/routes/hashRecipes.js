...
export function createHashRecipesRouter(db) {
    const router = Router();
    const createSchema = z.object({
        name: z.string().min(1).max(120),
        delimiter: z.string().optional(),
        fieldsJson: z.union([z.string(), z.array(z.string())]).transform((v) => typeof v === 'string' ? v : JSON.stringify(v)),
        normalizationJson: z.union([z.record(z.unknown()), z.string()]).optional().transform((v) => v === undefined ? undefined : typeof v === 'string' ? v : JSON.stringify(v))
    });
    const patchSchema = z.object({
        name: z.string().min(1).max(120).optional(),
        delimiter: z.string().optional(),
        fieldsJson: z.union([z.string(), z.array(z.string())]).optional().transform((v) => v === undefined ? undefined : typeof v === 'string' ? v : JSON.stringify(v)),
        normalizationJson: z.union([z.record(z.unknown()), z.string()]).optional().transform((v) => v === undefined ? undefined : typeof v === 'string' ? v : JSON.stringify(v))
    });
    ...
    router.post('/', requireAuth, requireRole('OWNER', 'ADMIN'), (req, res) => {
      ...
      VALUES (?, ?, ?, 1, ?, ?, ?, ?, ?, 0)
    `).run(id, orgId, parsed.data.name, parsed.data.delimiter ?? null, parsed.data.fieldsJson, parsed.data.normalizationJson ?? null, now, now);
    ...
    router.patch('/:id', requireAuth, requireRole('OWNER', 'ADMIN'), (req, res) => {
        ...
        const row = db.prepare('SELECT id, org_id, name, version, delimiter, fields_json, normalization_json, created_at, updated_at, active FROM hash_recipes WHERE id = ?').get(id);
        return res.json({
            id: row.id,
            orgId: row.org_id,
            name: row.name,
