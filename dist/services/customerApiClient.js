...
function buildAuthHeaders(config, method, url, body) {
    const out = {};
    const custom = config.headersJson ? JSON.parse(config.headersJson) : {};
...
export async function testHealth(config) {
    ...
    const run = async () => {
        return fetchWithTimeout(url, { method: 'GET', headers }, config.timeoutMs);
    };
    try {
...
export async function testStatus(config) {
...
export async function testDelete(config, requestId, subjectHash) {
    ...
    const start = Date.now();
    const run = async () => {
        return fetchWithTimeout(url, { method: 'POST', headers, body }, config.timeoutMs);
    };
    try {
        const res = await withRetries(run, config.retries);
        const latencyMs = Date.now() - start;
