 ...
 */
function ensureEvent(db, eventId, eventType, payloadSanitized) {
...
function markEventProcessed(db, eventId, status, errorMessage) {
    db.prepare(`
 ...
 */
export function applySetupFeePaymentFromCheckoutSession(db, session) {
    ...
    const existing = db.prepare('SELECT id, status FROM billing_payments WHERE stripe_checkout_session_id = ?').get(sessionId);
    const status = 'paid';
    if (existing) {
...
function handleCheckoutSessionExpired(db, session) {
    const sessionId = session.id;
    if (!sessionId)
        return;
    const now = nowIso();
    ...
    try {
        switch (event.type) {
            ...
            case 'payment_intent.payment_failed':
                handlePaymentIntentPaymentFailed(db, event.data.object);
                break;
            case 'checkout.session.expired':
                handleCheckoutSessionExpired(db, event.data.object);
                break;
            default:
                break;
        }
